...
if (!tienePermiso('equipos.eliminar')) {
    http_response_code(403);
    echo json_encode(['error' => 'No tiene permisos para eliminar equipos']);
    exit;
}
...
if ($componentesAsociados && $componentesAsociados['total'] > 0) {
    ...
    http_response_code(400);
    echo json_encode([
        'error' => 'No se puede eliminar el equipo porque tiene componentes asociados',
...
if ($mantenimientosAsociados && $mantenimientosAsociados['total'] > 0) {
    http_response_code(400);
    echo json_encode([
        'error' => 'No se puede eliminar el equipo porque tiene mantenimientos asociados',
...
if ($trabajoAsociado && $trabajoAsociado['total'] > 0) {
    http_response_code(400);
    echo json_encode([
        'error' => 'No se puede eliminar el equipo porque tiene registros de historial de trabajo asociados',
